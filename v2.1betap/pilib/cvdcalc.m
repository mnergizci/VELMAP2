function [maxvar,alpha] = cvdcalc(flatgrid,ncols,nlines,pixsizex,pixsizey,method,saved_acg)
...
% pixsizex/y = pixel size in kilometres
...
flatgrid(isnan(flatgrid))=0; %added by Hua Wang, 12/03/2012
if nargin<8
  if method==2
...
r = sqrt(((xx-xcent)*pixsizex).^2+((yy-ycent)*pixsizey).^2);
...
%    maxdist = ycent;
%end
w = max(pixsizex,pixsizey);
w = w*2;  %bin width
if (xcent*pixsizex<ycent*pixsizey)
    maxdist = xcent*pixsizex;
else
    maxdist = ycent*pixsizey;
end
