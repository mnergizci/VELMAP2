function [insar] = loadlics(insarpar, fullres)
%=============================================
...
% Muhammet Nergizci, some updates for sboi data and debug 26/04/2025
...
if nargin<2
 ...
 for i=insarpar.nsboifile:-1:1
    
    % read from parameter file if available, else read from insarpar
    procfile = [insarpar.dir{i} 'sboi.proc'];
    if exist(procfile,'file')
%         fprintf(' ')
        fprintf('%s exists, reading from file\n', procfile);
...
%         fprintf(' ')
        fprintf('%s doesn''t exist, use config values\n', procfile);
        insar(i).proc.orbdegree = insarpar.orbdegree;
        insar(i).proc.atmdegree = insarpar.atmdegree;
        insar(i).proc.invenu = insarpar.invenu;
        b = split(insarpar.dir{i}, '/');
        b(cellfun(@isempty, b)) = [];  % remove empty entries
        insar(i).proc.ID = b{2};       % safely extract the second valid part
        
    end
    fprintf('Working on SBOI data %d/%d from %s\n',i,insarpar.nsboifile,char(insarpar.dir{i}))

    %% ratemap

    fprintf('loading rate map ...\n');
    
    namestruct=dir([insarpar.dir{i} '*' insarpar.sboi_ext]);
    ...
    u(isnan(u)) = 0; % Replaces nan with zeros for SBOI
...
%    insar(i).vcm = sparse(double(diag(verr)));  %orig
    ...
    clear('errormap','verr');
    %%Print screen median_covariance
    vcm_temp = full(insar(i).vcm);       % Convert sparse to full
    vcm_temp(vcm_temp == 0) = NaN;       % Replace zeros with NaN
    mean_cov = mean(vcm_temp(:), 'omitnan');
    fprintf('Mean of covariance matrix: %.4f\n', mean_cov);
    fprintf('\n');
