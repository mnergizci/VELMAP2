function [insar] = loadlic(insarpar)
%=============================================
...
%stackmapname=fullfile(insardir,dir(strcat(insardir,'*.vel_deramp.mskd.geo.tif')).name);

namestruct=dir(string(strcat(insarpar.dir(i),'/*.vel_deramp.mskd.geo.tif')));
...
%-----------------------
 ...
 %efile=fullfile(insardir,dir(strcat(insardir,'*.geo.E.tif')).name);
 namestruct=dir(string(strcat(insarpar.dir(i),'/*.geo.E.tif')));
 efile=sprintf("%s/%s", string(insarpar.dir(i)),namestruct.name);
 [e]=tif2pi(efile,insar(i).ifghdr);
namestruct=dir(string(strcat(insarpar.dir(i),'/*.geo.N.tif')));
 nfile=sprintf("%s/%s", string(insarpar.dir(i)),namestruct.name);
 insar(i).proc.incfile=efile;
 [n]=tif2pi(nfile,insar(i).ifghdr);
namestruct=dir(string(strcat(insarpar.dir(i),'/*.geo.U.tif')));
...
%    errormapname=fullfile(insardir,dir(strcat(insardir,'*.vstd_deramp.mskd.geo.tif')).name);
    namestruct=dir(string(strcat(insarpar.dir(i),'/*.vstd_deramp.mskd.geo.tif')));
